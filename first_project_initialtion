# 《第一個專案初始化範例操作指南》

*(Project Demo Initialization Guide – From Zero to Working Build)*

---

## 一、目標

本文件將帶你從零開始建立一個完整可運行的專案：

> 🧩 由程式模組（Code）與提示詞模組（GEM）組裝成一個可執行的 AI 應用。

最終你將完成：

* 1 個 GEM 模組
* 1 個 Recipe 組裝檔
* 1 個執行程式（整合 Code + GEM）
* 1 個測試流程

---

## 二、專案初始化

### 1️⃣ 建立專案資料夾

在本地 VS Code 終端執行：

```bash
cd dev/projects
cp -r proj-demo proj-first
```

結構如下：

```
dev/projects/proj-first/
 ├─ app/
 ├─ prompts/
 │   ├─ recipes/
 │   ├─ compiled_prompts/
 │   └─ evals/
 ├─ ssot/
 └─ tests/
```

---

### 2️⃣ 建立 SSOT 架構（資料結構）

`ssot/schema.json`

```json
{
  "project_spec": {
    "intent": "string",
    "summary": "string",
    "report": "object"
  }
}
```

這份是所有模組與 prompt 共用的狀態模型。
它確保 Code ↔ GEM 之間溝通的欄位一致。

---

## 三、建立最小 GEM 模組

### 1️⃣ Component：功能模組

在 `core/prompts/components/` 新增
`fn_summarize_text.md`：

```markdown
---
name: fn_summarize_text
version: 1.0.0
reads: ["user_input"]
writes: ["project_spec.summary"]
contracts:
  output_type: string
---

請將以下輸入內容進行摘要，並以 JSON 格式輸出：
{
  "summary": "<一句話摘要>"
}
```

---

### 2️⃣ GEM：組合模組

在 `core/prompts/gems/` 新增
`gem_simple_summary.json`：

```json
{
  "name": "gem_simple_summary",
  "version": "1.0.0",
  "description": "基於單一摘要功能的簡易 GEM",
  "modules": [
    {"name": "fn_summarize_text", "version_range": ">=1.0.0"}
  ],
  "io_rules": {
    "authorize_writes": {
      "fn_summarize_text": ["project_spec.summary"]
    },
    "stage_order": ["summary"]
  }
}
```

---

## 四、建立 Recipe（專案組裝規格）

在 `dev/projects/proj-first/prompts/recipes/` 新增
`spec_summary_demo.json`：

```json
{
  "name": "spec_summary_demo",
  "use_gems": [
    {"name": "gem_simple_summary", "version": "1.0.0"}
  ]
}
```

這表示此專案要使用哪個 GEM 模組來組裝 prompt。

---

## 五、組裝與預覽 Prompt

### 1️⃣ 組裝命令

```bash
python core/prompts/tools/build_prompt.py \
  --recipe dev/projects/proj-first/prompts/recipes/spec_summary_demo.json \
  --registry core/prompts/registry
```

如果組裝成功，會輸出：

```
✅ Compiled prompt written to:
dev/projects/proj-first/prompts/compiled_prompts/spec_summary_demo.md
```

---

### 2️⃣ 預覽內容

使用：

```bash
python core/prompts/tools/prompt_preview.py \
  --recipe dev/projects/proj-first/prompts/recipes/spec_summary_demo.json
```

終端會印出：

```
=== RECIPE: spec_summary_demo ===
# GEM: gem_simple_summary@1.0.0
## [1] MODULE: fn_summarize_text v1.0.0
reads: ['user_input'] | writes: ['project_spec.summary']
contracts: {'output_type': 'string'}

請將以下輸入內容進行摘要，並以 JSON 格式輸出：
{ "summary": "<一句話摘要>" }
```

---

## 六、建立程式端整合（呼叫組裝結果）

### 1️⃣ 建立 `app/main.py`

```python
from pathlib import Path

def run_prompt():
    path = Path("dev/projects/proj-first/prompts/compiled_prompts/spec_summary_demo.md")
    prompt = path.read_text(encoding="utf-8")
    print("🧠 PROMPT COMPILED CONTENT:\n")
    print(prompt)

if __name__ == "__main__":
    run_prompt()
```

這只是讀取組裝結果，模擬後端呼叫 prompt 的情境。

---

### 2️⃣ 在 VS Code 執行

在命令面板 (Ctrl+Shift+B)：
選擇 `🚀 Run Demo App`

輸出：

```
🧠 PROMPT COMPILED CONTENT:
請將以下輸入內容進行摘要，並以 JSON 格式輸出：
{ "summary": "<一句話摘要>" }
```

---

## 七、測試與評估

新增 `tests/test_prompt_build.py`：

```python
from pathlib import Path

def test_compiled_prompt_exists():
    prompt_path = Path("dev/projects/proj-first/prompts/compiled_prompts/spec_summary_demo.md")
    assert prompt_path.exists(), "Compiled prompt should be generated"
    content = prompt_path.read_text()
    assert "摘要" in content
```

執行：

```bash
pytest dev/projects/proj-first/tests/
```

輸出：

```
✅ 1 passed in 0.12s
```

---

## 八、版本鎖定

在 `dev/projects/proj-first/prompts/` 新增：
`lockfile.json`

```json
{
  "project": "proj-first",
  "gems": {
    "gem_simple_summary": "1.0.0"
  },
  "compiled_at": "2025-11-06T11:00:00Z"
}
```

此檔案可確保之後即使 Core 更新，
此專案仍固定使用這個版本的 GEM。

---

## 九、Git 操作與同步

```bash
git add .
git commit -m "feat: add proj-first with simple summary gem"
git push origin main
```

CI/CD Pipeline（如 `.github/workflows/build.yml`）會自動：

1. 驗證 Recipe
2. 組裝 Prompt
3. 跑測試
4. 更新 `lockfile.json`

---

## 十、成果驗證

| 檢查項           | 結果 | 位置                                          |
| ------------- | -- | ------------------------------------------- |
| Prompt 是否成功組裝 | ✅  | `compiled_prompts/spec_summary_demo.md`     |
| GEM 是否可被引用    | ✅  | `core/prompts/gems/gem_simple_summary.json` |
| 測試是否通過        | ✅  | `tests/test_prompt_build.py`                |
| VS Code 可否執行  | ✅  | `app/main.py`                               |
| GitHub 是否同步   | ✅  | `main` branch                               |

---

## 🧠 十一、延伸實作

| 需求           | 作法                                   |
| ------------ | ------------------------------------ |
| 想加一個分析模組     | 新增 `fn_analyze_intent.md` → 更新 GEM   |
| 想讓程式自動呼叫 LLM | 在 `main.py` 實作 LLM API 呼叫            |
| 想加上回歸測試      | 在 `tests/eval_prompt_output.py` 比對結果 |
| 想重用此專案結構     | 直接 `cp -r proj-first proj-next`      |

---

## ✅ 十二、結論

你現在已經建立了：

* 一個最小 GEM
* 一個最小 Recipe
* 一次完整組裝
* 一次程式整合與測試

> 從這個基礎開始，你可以：
>
> * 增加多 GEM 組合
> * 建立完整的多階段 Pipeline
> * 擴展 Layer 層邏輯，形成共用框架

這個 `proj-first` 專案就成了你的第一個
**「模組化 AI 專案」實例** 🎯

---

是否要我接著幫你生成第五份文件：
📘《系統運行與維護手冊》（包含：版本治理、CI/CD、自動測試、部署策略）？
